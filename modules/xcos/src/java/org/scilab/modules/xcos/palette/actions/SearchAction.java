...
public class SearchAction extends CommonCallBack {
    ...
    protected static void search() {
        ...
        int n=txtSearch.getItemCount();
        SearchAction.checkingCombo(query,n);
        if (lastItem == null || !lastItem.toString().equals(query)) {
            txtSearch.insertItemAt(query, 0); // insert on top
        }
    }
    public static void checkingCombo(String str,int n) {
        for(int i=0;i<n;i++) {
            if(txtSearch.getItemAt(i).equals(str)) {
                txtSearch.removeItemAt(i);
                break;
            }
        }
    }
