...
public final class PaletteBlockKeyListener implements KeyListener {
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            control.getPaletteCtrl().addSelectedBlocks(theDiagram,size-1);
        ...
        //new  code for using shift combinations
    ...
    private void getNextBlock(KeyEvent e, int x, int y) {
        ...
        try {
            ...
            } else if (ctrlIsDown && e.getKeyCode() == KeyEvent.VK_SPACE) {
                nextBlockView.getController().setSelected(!isSelected);// toggle
            } 
            else if (ctrlIsDown && !isSelected) {
                nextBlockView.setStatusUI(StatusUI.HOVER);
            }
            else if(shiftIsDown) {
                boolean isSelected_cur=currentBlockView.getController().isSelected();
                if(isSelected_cur && isSelected)
                    currentBlockView.getController().setSelected(false);
                    nextBlockView.getController().setSelected(!isSelected);
                }
