...
SCICOS_BLOCKS_IMPEXP void cmscope(scicos_block * block, scicos_flag flag)
{
    ...
    switch (flag)
    {
        ...
        case StateUpdate:
            ...
            {
                u = (double *)block->inptr[i]; //tables of pointer to the regular input port
...
static void appendData(scicos_block * block, int input, double t, double *data)
{
    ...
    if (t > ((sco->scope.periodCounter[input] + 1) * block->rpar[1 + input]))
    {
        ...
        // insz is vector of sizes of regular input ports
    ...
    if (sco != NULL)
    {
        ...
        {
            double value = data[i];
            //printf("value(might be plotted)=%lf\n",value);
...
static void setFigureSettings(int iFigureUID, scicos_block * block)
{
    ...
    win_pos[0] = ipar[3];//window position
    win_pos[1] = ipar[4];//window sizes
    win_dim[0] = ipar[5];//input port sizes
    win_dim[1] = ipar[6];//colors
...
static BOOL setPolylinesBounds(scicos_block * block, int input, int periodCounter)
{
    ...
    //debug input=0, periodCounter=0
    dataBounds[0] = periodCounter * period; // xMin
    dataBounds[1] = (periodCounter + 1) * period;   // xMax
    //printf("block->nrpar=%d, nin=%d, input=%d\n",block->nrpar,nin,input);
